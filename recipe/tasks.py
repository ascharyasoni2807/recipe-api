...
def send_email(subject, body, to_email, from_email, smtp_server, smtp_port, smtp_user, smtp_password):
    ...
    try:
        ...
        logger.info(f'Email sent successfully by {from_email} to {to_email} ', extra={"AppName": "recipe"})
    except Exception as e:
        # Log the exception with traceback
        logger.error(f"Failed to send email: {e}", exc_info=True, extra={"AppName": "recipe"})
    finally:
        server.quit()
...
def send_daily_notifications():
    ...
    logger.info(f"Objects are {recipes}", extra={"AppName": "recipe"})
    for recipe in recipes:
        author = recipe.author
        likes = recipe.get_total_number_of_likes()
        logger.info(f"Likes are {likes}", extra={"AppName": "recipe"})
        if likes > 0:
